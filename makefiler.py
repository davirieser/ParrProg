...
if __name__ == "__main__":
    ...
    with open(os.path.join(conf_dir, "Makefile"), 'w') as f:
        ...
        for task_name, task_details in tasks:
            ...
            env_repl = [f"{task_name}_OPTIMIZATION_LEVEL=$(opt)", f"COMPILE_FILE={src_file}", f"EXE_FILE={remove_file_ending(src_file)}"]
            closing = " \\\n\t)"
            ...
            for env_name, _ in envs:
                ...
                env_repl.append(f"{task_name}_{env_name}=$({env_name})")
                closing = f" \\\n{indent_str})" + closing;
                indent += 1
            for flag, _ in flags:
                indent_str = '\t' * (indent - 1)
